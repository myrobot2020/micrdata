...
library(shiny)
ui <- fluidPage(
  selectInput("a","variable1",choices = names(df),selected = "urbanity"),
  selectInput("b","variable2",choices = names(df),selected="roof"),
  tableOutput("x"),
  tableOutput("y")
  )
...
server <- function(input, output, session) {
  ...
  r<-reactive({
    s<-table(df[[input$a]],df[[input$b]])
    s<-t(s)
    s<-as.data.frame.matrix(s)
    s$sum<-rowSums(s)
    s<-as.data.frame.matrix(s)
    
    
    s1<-prop.table(table(df[[input$a]],df[[input$b]]),margin = 2)
    s1<-t(s1)
    
    s1<-as.data.frame.matrix(s1)
    
    
    s2<-cbind(s,s1)
    s2<-as.data.frame.matrix(s2)
    
    #|> prop.table(s,margin = 1) |> as.data.frame.matrix()
    
    
    
    
    # s<-table(df[[input$a]],df[[input$b]]) |> t() |> as.data.frame.matrix()
    # s$sum<-rowSums(s)
    # s<-as.data.frame.matrix(s)
    #    s<-prop.table(table(df[[input$a]],df[[input$b]]),margin = 1) |> 
    #s<-cbind(s)
    #s<-as.data.frame.matrix(s)
    
    #s<-as.data.frame.matrix(cbind(prop.table(s,margin = 1),s))# |> )
    #s<-prop.table(s,margin = 1) |> 
    #s<-as.data.frame.matrix(s)
    #s<-cbind(prop.table(s[,1:ncol(s)-1],margin = 1),s) |> as.data.frame.matrix()
    # s1<-prop.table(s[,1:ncol(s)-1],margin = 1) |> as.data.frame.matrix() |> t()
    
    
  })
  
  output$x<-renderTable({r()},rownames = T)
