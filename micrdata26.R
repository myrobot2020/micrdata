...
library(shiny)
ui <- fluidPage(
  selectInput("a","variable",choices = names(df),selected = "urbanity"),
  uiOutput("b"),
  tableOutput("x"),
  tableOutput("y"),
  tableOutput("z")
...
server <- function(input, output, session) {
  ...
  output$b<-renderUI({
    selectInput("b","subvariable",choices = unique(df[,input$a]))
  })
  
  r<-reactive({
    df<-table(df[,input$a]) |> t() |> as.data.frame.matrix()
    df$sum<-rowSums(df)
  ...
  r1<-reactive({
    df<-prop.table(table(df[,input$a])) |> t() |> as.data.frame.matrix()
    #df$ptimesp<-df
  ...
  r2<-reactive({
    ...
    a<-print(paste("a",input$b))
    a[1]
    # df<-prop.table(table(df[,input$a])) |> t() |> as.data.frame.matrix()
    # df<-df[,!!sym(input$b)] |> as.data.frame.matrix()
    # #df$ptimesp<-df
    # 
    #df<- table(df[input$b,]) #|> as.data.frame.matrix()
    #df<-df[input$a,input$b]
  ...
  output$y<-renderTable({r1()})
  output$z<-renderTable({r2()})
}
