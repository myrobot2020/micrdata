...
#df$state<-substr(df$V1,1,2) %>% table()
#df$district<-substr(df$V1,3,4)%>% table()
#df$urbanity<-substr(df$V1,5,5)%>% table()
#df$ownership<-substr(df$V1,6,6)%>% table()
#df$housetype<-substr(df$V1,7,7)%>% table()
...
# 
# fie<-function(x){
#   colnames(x$urbanity)<-c("R","U")
#   
# }
# 
# fie<-function(x){
#   colnames(x$urbanity)<-code$urbanity[complete.cases(code$urbanity)]
#   }
# 
# 
# 
...
df$V1<-NULL
library(writexl)
df %>%  sample_n(1000000) #%>%   writexl::write_xlsx("C:/Users/HP/Desktop/India/uniindia.xlsx")
