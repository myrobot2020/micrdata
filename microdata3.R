...
library(dplyr)
library(purrr)
df0<-read.csv("C:/Users/HP/Desktop/India/sample data/SAMP_IND.txt", quote="\"",header = F)
code<- read_xls("C:/Users/HP/Desktop/India/annexure.xls")
df<-sample_n(df0,size = 10000,replace = T)
df$state<-substr(df$V1,1,2) %>% stri_replace_all_regex(pattern=sprintf('%0.2d', 1:35),replacement= code$State,vectorize_all = F)
...
df2<-df %>% map(table) %>% map(prop.table) %>% map(round,1) %>% map(fee<-function(x){x<-x*100}) 
