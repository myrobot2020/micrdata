...
ui <- fluidPage(
  fileInput("a","upload sample"),
  # uiOutput("b"), #create names from sample
  # uiOutput("c"), #create factors from subsetted sample
  # uiOutput("d"), #select variable for standard error
  # uiOutput("e"), #select factor for standard error
  tableOutput("f")
...
r1<-reactive({
    readxl::read_xls(input$a$datapath,header = F) |> fee()
  })
}  

output$f<-renderTable(r1())
...
fee<-function(x){
  x$urbanity<-substr(x$V1,5,5) |> stringi::stri_replace_all_regex(pattern=seq(1:2),replacement= code$urbanity,vectorize_all = F)
