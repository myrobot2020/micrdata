...
server <- function(input, output, session) {
  ...
  r<-reactive({
    ...
    s1<-prop.table(table(df[[input$a]],df[[input$b]]),margin = 2) |> t() |> as.data.frame.matrix()
    s2<-cbind(s,s1) |> as.data.frame.matrix()
    #s2$ptimesp1<-s2[ncol(s2)]*s2[ncol(s2)-1]
    s2$ptimesp1<-s2[ncol(s2)]*s2[ncol(s2)]
...
a<-tapply(df$electricity, df$roof, function(x){
prop <- prop.table(table(x))
SE <- sqrt(prop*(1-prop)/length(x))
l<-list(prop=prop, SE=SE)
}
)
